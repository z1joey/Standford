...
class PlayingCardView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        if isFaceUp {
            if rank > 10, let faceCardImage = UIImage(named: "swinburne", in: Bundle(for: self.classForCoder), compatibleWith: traitCollection) {
                faceCardImage.draw(in: bounds.zoom(by: faceCardScale))
            } else {
                drawPips()
            }
        } else {
            if let cardBackImage = UIImage(named: "swinburne", in: Bundle(for: self.classForCoder), compatibleWith: traitCollection) {
